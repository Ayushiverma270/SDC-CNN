...
class CarlaGame(object):
    def __init__(self, carla_client, args):
        ...
        self.steerModel = load_model()
        self.steerModel.load_weights('./img/model_th1.h5')
        self.throttleModel = load_model()
        self.throttleModel.load_weights('./img/model_tt1.h5')
        ...
        ## add brake as well first as in 0 and 1 then in parts to show the motion to be slowing down.. 
    ...
    def _on_loop(self):    # called in execute func
        ...
        if control is None:
            ...
            if self._main_image is not None:
                array = np.frombuffer(self._main_image.raw_data, dtype=np.dtype("uint8")) 
                array = np.reshape(array, (self._main_image.height, self._main_image.width, 4)) # RGBA format
                array = array[:, :, :3] #  Take only RGB
                
                control.steer = (self.steerModel.predict(np.array([array]))[0][0])
                
                if self._velocity > 80
                    throttle = 0
                else:
                    control.throttle = self.throttleModel.predict(np.array([array]))[0][0]
                print("steer" , control.steer)
                print("throttle ", control.throttle)
                print("speed ", self._velocity)
                
                
                #control.steer = self.trainedModel.predict(self._main_image)[0]
    ...
    def _on_render(self):                                         ## called in execute
        ...
        if self._main_image is not None:
            ...
            #print("\n")
            if self._velocity >= 0 and self._val3 == 1:   ## YOU CAN CHANGE THIS ACCORDING TO YOU
                print("\nimage is saved\n") #cv2.imwrite(array)
                self._data['data{}_angle_{}_throttle_{}_speed_{}_'.format(self._i,format(self._val1,'.2f'),format(self._val2,'.2f'),format(self._velocity,'.2f'))] = array
